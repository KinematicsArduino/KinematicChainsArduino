...
void MobileBase::parse() {
	...
	for (int i = 0; i < Limbs_size; i++) {
		...
		for (int j = 0; j < Limb_size; j++) {
			...
			double DH_alpha = doc.getMember(
					String(i) + "_DH_" + String(j))[0];
			double DH_d =
					doc.getMember(String(i) + "_DH_" + String(j))[1];
			double DH_r =
					doc.getMember(String(i) + "_DH_" + String(j))[2];
			double DH_theta = doc.getMember(
					String(i) + "_DH_" + String(j))[3];

			int PinNumber = doc.getMember(
					String(i) + "HardwareManager" + String(j));
			float offset = doc.getMember(
					String(i) + "Offset" + String(j));
			float scale = doc.getMember(String(i) + "scale" + String(j));
			Link *linkptr = new Link(j, DH_alpha, DH_d, DH_r, DH_theta,
					PinNumber, hwLocal, scale, offset);
			//add to limb array
